...
#Region "Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(Memberhelp_Room))
        ...
        Me.Label1.Text = "ROOM  OUTSTANDING"
        '
        'Shape1
        '
...
#Region "Upgrade Support "
    Private Shared m_vb6FormDefInstance As Memberhelp_Room
    Private Shared m_InitializingDefInstance As Boolean
    Public Shared Property DefInstance() As Memberhelp_Room
        Get
            If m_vb6FormDefInstance Is Nothing OrElse m_vb6FormDefInstance.IsDisposed Then
                m_InitializingDefInstance = True
                m_vb6FormDefInstance = New Memberhelp_Room
                m_InitializingDefInstance = False
            End If
            DefInstance = m_vb6FormDefInstance
        End Get
        Set(ByVal Value As Memberhelp_Room)
...
#End Region
    ...
    Private Sub Command1_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Command1.Click
        ...
        sSgrid.Focus()
    End Sub
    Private Sub Memberhelp_Room_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub MyFillGrid1()
        ...
        Else
            If optMCode.Checked = True Then
                SQLSTRING = "SELECT ISNULL(SUBNAME,'') AS SUBNAME,ISNULL(SUBCODE,'') AS SUBCODE FROM " & Trim(vUser & "") & "..SubledgerList_Room "
                SQLSTRING = SQLSTRING & " WHERE   SUBCODE LIKE '" & (Trim(txtSelection.Text)) & "%' "
                SQLSTRING = SQLSTRING & " AND ACCODE='" & Trim(ROOMCODE) & "' ORDER BY SUBCODE "
            ...
            End If
        End If
        Vconn.getDataSet(SQLSTRING, "SubledgerList_Room")
        Dim loopindex As Integer
        If gdataset.Tables("SubledgerList_Room").Rows.Count > 0 Then
            If grdSelectionList.DataRowCnt < gdataset.Tables("SubledgerList_Room").Rows.Count - 1 Then
                grdSelectionList.MaxRows = gdataset.Tables("SubledgerList_Room").Rows.Count + 3
            End If
            For loopindex = 0 To gdataset.Tables("SubledgerList_Room").Rows.Count - 1
                grdSelectionList.Col = 1
                grdSelectionList.Row = loopindex + 1
                grdSelectionList.Text = CStr(gdataset.Tables("SubledgerList_Room").Rows(loopindex).Item("Subcode"))
                grdSelectionList.Col = 2
                grdSelectionList.Row = loopindex + 1
                grdSelectionList.Text = CStr(gdataset.Tables("SubledgerList_Room").Rows(loopindex).Item("SubName"))
            Next
        Else
            lblAmount.Text = ""
    ...
    Private Sub GetMemAcc()
        ...
        End If
    End Sub
    Private Sub Memberhelp_Room_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        If FormUnload Then
            Me.Hide()
        End If
    End Sub
    Private Sub Memberhelp_Room_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Dim gConnection As Object
        lblType.Text = ""
        lblname.Text = ""
        lblmcode.Text = ""
    ...
    Private Sub Label5_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Label5.Click
        Me.Close()
        Memberhelp_Room.DefInstance = Nothing
    End Sub
    ...
    Private Sub FillGrid1()
        ...
        vTotal = 0
        Call GetOpBal()
        sstr = "SELECT ISNULL(RECEIPTNO,0) AS RECNO,* FROM  " & Trim(vUser & "") & "..JOURNALENTRY WHERE ACCOUNTCODE =  '" & Trim(ROOMCODE) & "' AND SLCODE= '" & Trim(vMemAcc) & "' AND  VOUCHERDATE >= '" & Format(Mskfrom.Value, "dd/MMM/yyyy") & "' AND VOUCHERDATE <= '" & Format(Mskto.Value, "dd/MMM/yyyy") & "'"
    ...
    Private Sub GetOpBal()
        ...
        ssql = "SELECT ISNULL(CREDITDEBIT,'') AS  CREDITDEBIT,SUM(AMOUNT) AS AMT FROM " & Trim(vUser & "") & "..OUTSTANDING WHERE SLCODE = '" & Trim(vMemAcc) & "' AND VOUCHERDATE < '" & Format(Mskfrom.Value, "dd-MMM-yyyy") & "' AND ACCOUNTCODE='" & Trim(ROOMCODE) & "'  GROUP BY CREDITDEBIT"
    ...
    Private Sub MyFillGrid()
        ...
        Else
            If optMCode.Checked = True Then
                vSql = "SELECT ISNULL(SUBNAME,'') AS SUBNAME,ISNULL(SUBCODE,'') AS SUBCODE FROM " & Trim(vUser & "") & "..SubledgerList_Room "
                vSql = vSql & " WHERE   SUBCODE LIKE '" & (Trim(txtSelection.Text)) & "%' "
                vSql = vSql & " AND ACCODE='" & Trim(ROOMCODE) & "' ORDER BY SUBCODE "
            ElseIf optAccName.Checked Then
                vSql = "SELECT ISNULL(SUBNAME,'') AS SUBNAME,ISNULL(SUBCODE,'') AS SUBCODE FROM " & Trim(vUser & "") & "..SubledgerList_Room "
            ...
            End If
        End If
        Vconn.getDataSet(vSql, "SubledgerList_Room")
        Dim loopindex As Integer
        If gdataset.Tables("SubledgerList_Room").Rows.Count > 0 Then
            If grdSelectionList.DataRowCnt < gdataset.Tables("SubledgerList_Room").Rows.Count - 1 Then
                grdSelectionList.MaxRows = gdataset.Tables("SubledgerList_Room").Rows.Count + 3
    ...
    Private Sub grdSelectionList_LeaveCell(ByVal sender As Object, ByVal e As AxFPUSpreadADO._DSpreadEvents_LeaveCellEvent) Handles grdSelectionList.LeaveCell
        ...
        If OptOthers.Checked = False Then
            ...
            vSql = vSql & " FROM " & Trim(vUser) & "..SubledgerList_Room WHERE SUBCODE ='" & Trim(grdSelectionList.Text) & "'"
            Vconn.getDataSet(vSql, "SubledgerList_Room")
            If gdataset.Tables("SubledgerList_Room").Rows.Count > 0 Then
                lblmcode.Text = IIf(CStr(gdataset.Tables("SubledgerList_Room").Rows(0).Item("Subcode")) <> "" Or Not IsDBNull(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubCode")), Trim(CStr(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubCode"))), " ")
                lblname.Text = IIf(CStr(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubName")) <> "" Or Not IsDBNull(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubName")), Trim(CStr(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubName"))), " ")
                FraDependents.Visible = False
                fraDetails.Visible = False
                vSql = "SELECT PHOTOIMAGE AS PHOTOIMAGE  FROM " & Trim(vUser) & "..PHOTOADDING WHERE MCODE = '" & Trim(lblmcode.Text) & "'"
                Vconn.getDataSet(vSql, "PHOTOADDING")
                ...
                If Trim(Vdesc) <> "" Then
                    gPicture = Trim(Vdesc & "")
                End If
                vMemberCode = gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubCode")
                vMemberCode = IIf(CStr(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubCode")) <> "" Or Not IsDBNull(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubCode")), Trim(CStr(gdataset.Tables("SubledgerList_Room").Rows(0).Item("SubCode"))), " ")
                vBal = Val(gdataset.Tables("SubledgerList_Room").Rows(0).Item("ClosingBal") & "")
                If vBal > 0 Then
