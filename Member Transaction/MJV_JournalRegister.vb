...
#Region " Windows Form Designer generated code "
    ...
    Friend WithEvents CHK_OTHERS As System.Windows.Forms.CheckBox
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.CHK_OTHERS = New System.Windows.Forms.CheckBox
        ...
        Me.GroupBox1.Controls.Add(Me.CHK_OTHERS)
        ...
        'CHK_OTHERS
        '
        Me.CHK_OTHERS.BackColor = System.Drawing.Color.Transparent
        Me.CHK_OTHERS.Checked = True
        Me.CHK_OTHERS.CheckState = System.Windows.Forms.CheckState.Checked
        Me.CHK_OTHERS.Font = New System.Drawing.Font("Times New Roman", 12.0!, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
        Me.CHK_OTHERS.Location = New System.Drawing.Point(312, 56)
        Me.CHK_OTHERS.Name = "CHK_OTHERS"
        Me.CHK_OTHERS.TabIndex = 447
        Me.CHK_OTHERS.Text = "OTHERS"
        '
        'CHK_POS
        '
        Me.CHK_POS.BackColor = System.Drawing.Color.Transparent
        Me.CHK_POS.Checked = True
        Me.CHK_POS.CheckState = System.Windows.Forms.CheckState.Checked
        Me.CHK_POS.Font = New System.Drawing.Font("Times New Roman", 12.0!, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
        Me.CHK_POS.Location = New System.Drawing.Point(128, 56)
        Me.CHK_POS.Name = "CHK_POS"
        Me.CHK_POS.Size = New System.Drawing.Size(72, 24)
        Me.CHK_POS.TabIndex = 446
        Me.CHK_POS.Text = "POS"
        '
        'Dtp_FromVoucherDate
        '
...
#End Region
    Private Sub MJV_JournalRegister_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            SQLSTRING = "select FROMDATE,TODATE FROM POS_FUND_MASTER"
            VCONN.getDataSet(SQLSTRING, "POS_FUND_MASTER")
            If gdataset.Tables("POS_FUND_MASTER").Rows.Count > 0 Then
                Dtp_FromVoucherDate.Text = Format(gdataset.Tables("POS_FUND_MASTER").Rows(0).Item("FROMDATE"), "dd/MM/yyyy")
                Dtp_ToVoucherDate.Text = Format(gdataset.Tables("POS_FUND_MASTER").Rows(0).Item("TODATE"), "dd/MM/yyyy")
            End If
    ...
    Private Sub GridOperation()
        Try
            ...
            Dim OTHERSFLAG As String
            Dim POSFLAG As String
            ...
            If CHK_OTHERS.Checked = True Then
                OTHERSFLAG = "Y"
            Else
                OTHERSFLAG = "N"
            End If

            strsql = "EXEC MJV_POSTING '" & Format(Dtp_ToVoucherDate.Value, "dd-MMM-yyyy") & "','T','" & Trim(POSFLAG) & "','" & Trim(OTHERSFLAG) & "'"
    ...
    Private Sub VIEWOPERATION()
        Try
            ...
            If ssGrid.DataRowCnt > 1 Then
                ...
                ROWCOUNT = ROWCOUNT + 1
                Filewrite.WriteLine("{0,-30}{1,-26}{2,-30}{3,-25}{4,-24}", Mid(Address1, 1, 30), " ", Mid(Trim(vcaption1), 1, 30), " ", "01-04-" & gFinancialyearStart & " TO 31-03-" & gFinancialyearEnd)
                ROWCOUNT = ROWCOUNT + 1
    ...
    Private Sub cmd_AccPosting_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_AccPosting.Click
        Try
            ...
            SQLSTRING = "select distinct accountcode,ACCOUNTCODEDESC,SLCODE,SLDESC,COSTCENTERCODE FROM purchase_journal_entry  "
            ...
            If gdataset.Tables("ACCODE").Rows.Count > 0 Then
                For I = 0 To gdataset.Tables("ACCODE").Rows.Count - 1
                    With GrdAuditTrail
                        ...
                        If Trim(gdataset.Tables("ACCODE").Rows(I).Item("COSTCENTERCODE")) <> "" Then
                            .Text = Trim(gdataset.Tables("ACCODE").Rows(I).Item("COSTCENTERCODE"))
                        Else
                            .Text = ""
                        End If
    ...
    Private Sub CMD_POST_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMD_POST.Click
        Try
            ...
            STRSQL = "SELECT ISNULL(COUNT(*),0) AS COUNT FROM MEMBER_MONTH_BILL WHERE  ISNULL(SUBSCRIPTIONTYPE,'')='' AND ISNULL(BILLDESC,'')='' AND MONTH(BILLDATE)=" & Month(Dtp_ToVoucherDate.Value)
            VCONN.getDataSet(STRSQL, "JournalEntry")
            If Val(gdataset.Tables("JournalEntry").Rows(0).Item("COUNT")) > 0 Then
                MsgBox("Billing and Accounts Mapping is Not Done Properly....")
                ' Exit Sub
            End If

            STRSQL = "SELECT CREDITDEBIT,ISNULL(SUM(ISNULL(AMOUNT,0)),0) AS AMOUNT FROM PURCHASE_JOURNAL_ENTRY GROUP BY CREDITDEBIT "
            VCONN.getDataSet(STRSQL, "JournalEntry")
            If gdataset.Tables("JournalEntry").Rows.Count > 0 Then
                For I = 0 To gdataset.Tables("JournalEntry").Rows.Count - 1
                    If gdataset.Tables("JournalEntry").Rows(I).Item("CREDITDEBIT") = "CREDIT" Then
                        CRAMT = Val(gdataset.Tables("JournalEntry").Rows(I).Item("AMOUNT"))
                    Else
                        DRAMT = Val(gdataset.Tables("JournalEntry").Rows(I).Item("AMOUNT"))
                    End If
                Next
                If CRAMT <> DRAMT Then
                    MsgBox("CREDIT AND DEBIT NOT TALLIED...")
                    Exit Sub
                End If
            Else
            ...
            'STRSQL = "SELECT ISNULL(CLOSED,'N') FROM MONTHCLOSE_POS WHERE MONTHNO=" & Month(Dtp_ToVoucherDate.Value)
            'VCONN.getDataSet(STRSQL, "MONTHCLOSE")
            'If gdataset.Tables("MONTHCLOSE").Rows.Count > 0 Then
            '    If gdataset.Tables("MONTHCLOSE").Rows(0).Item("CLOSED") = "Y" Then
            ...
            Dim OTHERSFLAG, POSFLAG As String
            If CHK_POS.Checked = True Then
                POSFLAG = "Y"
            Else
                POSFLAG = "N"
            ...
            '            STRSQL = "EXEC MJV_POSTING '" & Format(Dtp_ToVoucherDate.Value, "dd-MMM-yyyy") & "','P','" & Trim(POSFLAG) & "','" ^ Trim(OTHERSFLAG) & "'"
            STRSQL = "EXEC MJV_POSTING '" & Format(Dtp_ToVoucherDate.Value, "dd-MMM-yyyy") & "','P','" & Trim(POSFLAG) & "','" & Trim(OTHERSFLAG) & "'"


            '            STRSQL = "EXEC MJV_POSTING '" & Format(Dtp_ToVoucherDate.Value, "dd-MMM-yyyy") & "','P'"
            VCONN.dataOperation(6, STRSQL, "ACCOUNTPOSTING")
            Me.Cursor = Cursors.Default
