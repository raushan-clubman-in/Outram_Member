...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Label8.Location = New System.Drawing.Point(40, 456)
        ...
        Me.txt_Total.Location = New System.Drawing.Point(864, 456)
        ...
        Me.Label9.Location = New System.Drawing.Point(752, 456)
        ...
        Me.ssgrid_Fac.Size = New System.Drawing.Size(944, 256)
        ...
        Me.cbo_paymentmode.Location = New System.Drawing.Point(176, 456)
...
#End Region
    ...
    Private Sub txtmembercode_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtmembercode.Validated
        Dim sqlstring, totalamt, Luxurytax, totaltax, Taxamount, totaldisc, totalsubs, Totalamount, sqlstring1 As String
        Dim j As Integer
        ...
        Try
            If Trim(txtmembercode.Text) <> "" Then
                ...
                If gdataset.Tables("MEMBER").Rows.Count > 0 Then
                    ...
                    sqlstring = "select ISNULL(m.membertype,'') as membertype,ISNULL(M.SUBSCODE,'') AS SUBSCODE,t.subtypedesc,ISNULL(S.SUBSDESC,'') AS SUBSDESC,s.subsacctin,ISNULL(S.TOTAL,0) AS TOTAL,ISNULL(S.TAXTOTAL,0)AS TAXTOTAL,ISNULL(S.Luxurytax,0)AS Luxurytax from membertypedtl m, subcategorymaster t, subscriptionmast s where m.membertype=t.subtypecode and m.subscode=s.subscode and m.membertype=s.subscode and t.subtypecode='" & Trim(MEBERTYPECODE.Text) & "'"
                    gconnection.getDataSet(sqlstring, "SUBS")
                    If gdataset.Tables("SUBS").Rows.Count > 0 Then
                        With ssgrid_Fac
                            For j = 0 To gdataset.Tables("SUBS").Rows.Count - 1
                                ...
                                .Row = j + 1
                                .Lock = True
                                .Text = gdataset.Tables("SUBS").Rows(j).Item("Luxurytax")
                                Luxurytax = Val(.Text)
                                .Col = 7
                                .Row = j + 1
                                .Lock = True
                                ...
                                totalamt = Val(.Text)
                                .SetText(9, j + 1, totalamt)
                                Totalamount = Format((Val(Totalamount) + Val(Luxurytax) * Val(MonthsDiff)) + Val(totalamt), "0.00")
                                .Col = 10
                                .Row = j + 1
                                .Lock = True
    ...
    Private Sub cmdadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdadd.Click
        ...
        If cmdadd.Text = "Add New[F5]" Then
            ...
            For i = 1 To ssgrid_Fac.DataRowCnt Step 1
                strSQL = "INSERT INTO Subscriptionreceipts(ReceiptsNo,ReceiptsDate,SLCode,Slname,Membertype,PreviousDate_From,PreviousDate_To,FromDate,ToDate,SubsCODE,SubsDESC,SubsAmount,Luxurytax,Taxamount,Discount,Amount,Totalamount,PaymentMode,Instrumentno,InstrumentDate,BankName,Bankplace,Freeze)"
                ...
                strSQL = strSQL & Trim(txt_bankname.Text) & "','" & Trim(Txt_Bankplace.Text) & "','N')"
    ...
    Private Sub Calculate()
        Dim MonthsDiff As Integer
        Dim Fromdate, Todate As Date
        Dim total, totaldisc, totalsubs, Luxurytax, Taxamount, totaltax, Totalamount As Integer
        Dim i, j As Integer
        Me.txt_Total.Text = "0.00"
        With ssgrid_Fac
            For i = 1 To .DataRowCnt
                ...
                If Val(totalsubs) > 0 Then
                    Totalamount = (((Val(totalsubs) + Val(Luxurytax)) * Val(MonthsDiff)) - Val(totaldisc))
                End If
                .Col = 10
                .Row = i
                .Text = (Val(Totalamount) + Val(totaltax))
    ...
    Private Sub Txt_ReceiptsNo_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Txt_ReceiptsNo.Validated
        ...
        Try
            If Trim(Txt_ReceiptsNo.Text) <> "" Then
                ssgrid_Fac.ClearRange(1, 1, -1, 1 - 1, True)
                sqlstring = "Select isnull(ReceiptsNo,'') as ReceiptsNo,isnull(ReceiptsDate,'') as ReceiptsDate ,isnull(PreviousDate_from,'') as PreviousDate_from,isnull(PreviousDate_To,'')as PreviousDate_To,isnull(FromDate,'') as FromDate,isnull(ToDate,'') as ToDate,isnull(Slcode,'')as Slcode,isnull(Slname,'') as Slname,isnull(subsCode,'') as subsCode,isnull(Subsdesc,'') as Subsdesc,isnull(Subsamount,0) as Subsamount,isnull(Luxurytax,0) as Luxurytax,isnull(TaxAmount,0) as TaxAmount,isnull(Discount,0)as Discount,isnull(Amount,0) as Amount,isnull(TotalAmount,0) as TotalAmount,isnull(Paymentmode,'') as Paymentmode,isnull(Instrumentno,'')as Instrumentno,isnull(InstrumentDate,'') as InstrumentDate,isnull(Bankname,'') as Bankname,isnull(BankPlace,'') as Bankplace,isnull(Membertype,'') as Membertype,isnull(Freeze,'') as Freeze,AddDateTime from Subscriptionreceipts where ReceiptsNo='" & Trim(Txt_ReceiptsNo.Text) & "'"
                gconnection.getDataSet(sqlstring, "SUBS")
                If gdataset.Tables("SUBS").Rows.Count > 0 Then
