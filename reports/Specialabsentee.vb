...
Imports System.IO
Public Class Specialabsentee
    Inherits System.Windows.Forms.Form
    Dim FltrStr, status1, TempString(2), DispString, addFltrStr As String
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(Specialabsentee))
        ...
        Me.GroupBox1.Location = New System.Drawing.Point(240, 104)
        ...
        Me.ChK_Memberstatus.Location = New System.Drawing.Point(338, 112)
        ...
        Me.chk_PrintAll.Location = New System.Drawing.Point(86, 80)
        ...
        Me.chk_Filter_Field.Location = New System.Drawing.Point(62, 112)
        ...
        Me.chk_STATUS.Location = New System.Drawing.Point(389, 80)
        ...
        Me.Label3.Location = New System.Drawing.Point(386, 64)
        ...
        Me.cmd_MCodeToHelp.Location = New System.Drawing.Point(400, 360)
        ...
        Me.cmd_MCodefromHelp.Location = New System.Drawing.Point(270, 360)
        ...
        Me.Label7.Location = New System.Drawing.Point(158, 360)
        ...
        Me.Label8.Location = New System.Drawing.Point(312, 360)
        ...
        Me.tbx_filter_To.Location = New System.Drawing.Point(336, 360)
        ...
        Me.tbx_Filter_From.Location = New System.Drawing.Point(206, 360)
        ...
        Me.cmb_Filter_By.Location = New System.Drawing.Point(264, 16)
        ...
        Me.Label9.Location = New System.Drawing.Point(104, 16)
        ...
        Me.Label10.Location = New System.Drawing.Point(920, 136)
        ...
        Me.Cmb_add_filter.Location = New System.Drawing.Point(928, 160)
        Me.Cmb_add_filter.Name = "Cmb_add_filter"
        Me.Cmb_add_filter.Size = New System.Drawing.Size(136, 26)
        Me.Cmb_add_filter.TabIndex = 417
        ...
        Me.Label1.Location = New System.Drawing.Point(448, 72)
        Me.Label1.Name = "Label1"
        Me.Label1.Size = New System.Drawing.Size(267, 26)
        Me.Label1.TabIndex = 420
        Me.Label1.Text = "SPECIAL ABSENTEE LIST"
        '
        'GroupBox2
        '
        ...
        Me.GroupBox2.Location = New System.Drawing.Point(280, 528)
        ...
        Me.KeyPreview = True
        Me.Name = "Specialabsentee"
        Me.Text = "Specialabsentee"
...
#End Region
    Private Sub Specialabsentee_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cmb_Filter_By.Focus()
    ...
    Private Sub GetRights()
        ...
        End If
    End Sub
    Private Sub Specialabsentee_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub cmb_Filter_By_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_Filter_By.SelectedIndexChanged
        ...
        'End Try
    End Sub
    Private Sub cmb_Filter_By_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmb_Filter_By.KeyPress
        ...
        Try
            ...
            For i = 0 To ChK_Memberstatus.CheckedItems.Count - 1
                If i = 0 Then
                    ...
                    status1 = TempString(1)
                Else
                    TempString = Split((ChK_Memberstatus.CheckedItems.Item(i)), ".")
                    status1 = status1 & "','" & TempString(1)
    ...
    Private Sub lockerlist()
        ...
        Dim PrintStr(20), fromDt, toDt, STATUS, mcode As String
        Dim LP As Integer = 8
        Dim cnt As Integer
        Dim dr1 As DataRow
        Dim dr2 As DataRow
        ...
        Try
            ...
            End If
            Call chkfilterfield()
            sqlstring = "select ISNULL(MCODE,'') AS MCODE,ISNULL(MNAME,'') AS MNAME, ISNULL(CONTADD1,'') AS CONTADD1,ISNULL(CONTADD2,'') AS CONTADD2,ISNULL(CONTADD3,'') AS CONTADD3,ISNULL(CONTSTATE,'') AS CONTSTATE,ISNULL(CONTPIN,'') AS CONTPIN,ISNULL(effectivefrom,'') AS effectivefrom,ISNULL(effectiveto,'') AS effectiveto, ISNULL(NEWSTATUS,'') AS STATUS FROM specialabsentee where mcode  in (select mcode from membermaster where membertypecode in ('" & addFltrStr & "')" & " and curentstatus in('" & status1 & "'))"
            ...
            End If
            sqlstring = sqlstring & " ORDER BY MCODE,EFFECTIVEFROM"
            ...
            If Len(DispString) <= 58 Then
                Filewrite.WriteLine(Chr(27) + "E" & "SPECIAL ABSENTEE LIST" & Trim(DispString) & Chr(27) + "F")
                pagesize = pagesize + 1
            Else
                Filewrite.Write(Chr(27) + "E" & "SPECIAL ABSENTEE LIST")
                j = 0
            ...
            Filewrite.WriteLine("|SNo|MCODE    |MEMBER NAME                  |STATUS   |FROM DATE     |TOTADTE |")
            Filewrite.WriteLine(StrDup(80, "-"))
            SNO = 1
            If addlist.Rows.Count > 0 Then
                mcode = Trim(addlist.Rows(row).Item("MCODE"))
                fromDt = Trim(addlist.Rows(row).Item("effectivefrom"))
                toDt = Trim(addlist.Rows(row).Item("effectiveto"))
                STATUS = Trim(addlist.Rows(row).Item("STATUS"))
                PrintStr(1) = Trim(addlist.Rows(row).Item("MCODE"))
                PrintStr(2) = Trim(addlist.Rows(row).Item("MNAME"))
                PrintStr(3) = Mid(Trim(addlist.Rows(row).Item("CONTADD1")), 1, 39)
                PrintStr(4) = Trim(addlist.Rows(row).Item("CONTADD2"))
                PrintStr(5) = Trim(addlist.Rows(row).Item("CONTADD3"))
                PrintStr(6) = Trim(addlist.Rows(row).Item("CONTSTATE")) & "PIN:" & Trim(addlist.Rows(row).Item("CONTPIN"))
                For row = 0 To addlist.Rows.Count - 1
                    If pagesize > 60 Then
                        ...
                        Filewrite.WriteLine(StrDup(80, "-"))
                    End If
                    If mcode <> Trim(addlist.Rows(row).Item("MCODE")) Then
                        Filewrite.WriteLine(StrDup(80, "-"))
                        pagesize = pagesize + 1
                        fromDt = Trim(addlist.Rows(row).Item("effectivefrom"))
                        toDt = Trim(addlist.Rows(row).Item("effectiveto"))
                        ...
                        SNO = SNO + 1
                        PrintStr(0) = SNO
                        str = Trim(addlist.Rows(row).Item("mcode"))
                        Filewrite.WriteLine("{0,-5}{1,-39}{2,-15}{3,-15}", PrintStr(0), PrintStr(1) & ": " & PrintStr(2), "", "")
                        pagesize = pagesize + 1
                        LP = 3
                        Filewrite.WriteLine("{0,-5}{1,-39}{2,-10}{3,-15}{4,-15}", "", PrintStr(LP), STATUS, fromDt, toDt)
                        pagesize = pagesize + 1
                        LP = LP + 1
                    Else
                        fromDt = Trim(addlist.Rows(row).Item("effectivefrom"))
            ...
            End If
            Filewrite.WriteLine(StrDup(80, "-"))
            Filewrite.WriteLine("NO. OF SPECIAL ABSENTEE MEMBERS =" & Trim(CStr(SNO)) & Space(5) & "USER NAME: " & gUsername & Space(7) & "DATE :" & Format(Date.Now, "dd/MM/yyyy"))
    ...
    Private Sub tbx_filter_To_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles tbx_filter_To.KeyPress
        ...
        End If
    End Sub
    Private Sub cmb_Filter_By_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmb_Filter_By.KeyDown
